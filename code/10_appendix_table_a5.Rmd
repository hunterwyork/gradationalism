...
library(kableExtra)
table3 <- table3[,.( ll, bic, df, df.x, Chisq.x,
...
table3 <- table3[,.(model, ll, bic, df, df.x, Chisq.x, `Pr(>Chisq).x`, df.y, Chisq.y, `Pr(>Chisq).y`,  df.z, Chisq.z, `Pr(>Chisq).z`,  df.a, Chisq.a, `Pr(>Chisq).a`)]

nums <- names(table3)[names(table3) %like% "aic|bic|df|^Chisq|ll"]
...
knitr::kable(table3,"latex", row.names = F,longtable = T, escape = F,align = c("l", rep("r", 5),
